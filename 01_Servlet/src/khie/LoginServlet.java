 ...
 * 3. 그리고 난 후 Servlet의 goGet() 메서드나 doPost() 메서드를 호출하면서
 * 		이 객체들을 전달하게 됨.
 * 4. 톰캣이 사용자의 요청에 대한 정보를 모든 HttpServletRequest 객체의 속성으로
 * 		담아 메서드로 전달을 함. 따라서 HttpServletRequest 에서 제공하는 메서드들은
 * 		매개변수로 넘어온 객체들을 이용하여 사용자가 전송한 데이터를 받아오거나 응답을 할 수 있는 것임.
 * 		
 * 서블릿에서 클라이언트의 요청을 얻는 방법
 * - HttpservletRequest 객체에서 <form> 태그로 전송된 데이터를 받아오는데
 * 	사용이 되는 메서드.
 * 	* getParameter(String name) : <form> 태그의 name 속성에 들어간 변수명을 받아서 사용을 함. 반환형은 String 타입임.
 * 
 * 	* getparameterValues(String name) : <form> 태그와 같은 name에 대하여 여러 개의 값을 얻을 때 사용함.
 * 										반환형은 String[] 배열 타입임.
 * 
 ...
 *  4. setContentType() 메서드를 이용하여 클라이언트에 전송할 데이터의 종류
 *  	(MIME-TYPE)를 지정함.
 *  5. 클라이언트(웹 브라우저)와 서블릿의 통신은 잡 I/O 스트림을 이용함.
 *  
